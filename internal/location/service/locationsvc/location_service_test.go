...
func TestSetLocationByDriverId(t *testing.T) {
	...
	mockRepo.On("SetLocationByDriverId", mock.Anything, "driver123", float32(1.0), float32(2.0)).Return(nil)
...
func TestGetDriversInLocation(t *testing.T) {
	...
	mockRepo.On("GetDriversInLocation", mock.Anything, float32(1.0), float32(2.0), float32(3.0)).
		Return([]model.Location{{Lat: 1.0, Lng: 2.0}}, nil)

	locationService := New(context.Background(), mockRepo)

	result, err := locationService.GetDriversInLocation(context.Background(), float32(1.0), float32(2.0), float32(3.0))

	assert.NoError(t, err, "Expected no error")
	assert.NotNil(t, result, "Expected result not to be nil")
	assert.Equal(t, float32(1.0), result[0].Lat)
	assert.Equal(t, float32(2.0), result[0].Lng)
