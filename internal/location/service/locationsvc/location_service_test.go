...
func TestSetLocationByDriverId(t *testing.T) {
	...
	mockRepo.On("SetLocationByDriverId", mock.Anything, "driver123", 1.0, 2.0).Return(nil)
...
func TestGetDriversInLocation(t *testing.T) {
	...
	mockRepo.On("GetDriversInLocation", mock.Anything, 1.0, 2.0, 3.0).
		Return([]model.Location{{Lat: 1.0, Lng: 2.0}}, nil)

	locationService := New(context.Background(), mockRepo)

	result, err := locationService.GetDriversInLocation(context.Background(), 1.0, 2.0, 3.0)

	assert.NoError(t, err, "Expected no error")
	assert.NotNil(t, result, "Expected result not to be nil")
	assert.Equal(t, 1.0, result[0].Lat)
	assert.Equal(t, 2.0, result[0].Lng)
